		...
		var methods = {
			init: function(options) {
				...
				return this.each(function() {
					...
					$tree.addClass('explr-tree').css({
					...
					}).find('li:not(:last-child) > ul').addClass('explr-line-fix').end().find('li').prepend('<span class="explr-line"></span>').not(':has(ul)').addClass('tree-text').end().has('ul').prepend('<span class="explr-plus" title="' + opts.folderTooltip + '"></span>').delegate('.explr-plus, a[href="#"]', 'click', function(e) {
						...
						}).siblings('.explr-plus').toggleClass('explr-minus');
						return false
					});
					...
					if (helpers.cookie('check', treeId, false, opts.rememberState)) {
						...
						$tree.find('ul').hide().end().find('.explr-expand').show().siblings('.explr-plus').addClass('explr-minus')
					} else {
						$tree.find('.explr-collapse').hide().end().find('ul:not(.explr-collapse)').siblings('.explr-plus').addClass('explr-minus')
					};
					if ($.browser.msie) {
